var NSV = (function(NSV, $, undefined) {
    ...
    NSV.div_ad_gen = function () {
        ...
        if (BRS.accountInfo.unconfirmedBalanceNQT) {
            ...
            if (balance < len) {
                err_message = "You don't have enough BNC in this account for this distribution. ".concat(String(len),"BNC needed.");
            }
        }
        else {
    ...
    NSV.getAccountError = function(accountId, callback) {
        BRS.sendRequest("getAccount", {
            ...
            if (response.publicKey) {
                callback({
                    "type": "info",
                    "message": "The recipient account has a public key and a balance of " + BRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " BNC.",
                    "account": response
                });
            }
            else {
                if (response.errorCode) {
                    if (response.errorCode == 4) {
                        callback({
                            "type": "danger",
                            "message": "The recipient account is malformed, please adjust." + (!(/^(BNC\-)/i.test(accountId)) ? " If you want to type an alias, prepend it with the @ character." : ""),
                            "account": null
                        });
                    }
                    ...
                    callback({
                        "type": "warning",
                        "message": "The recipient account does not have a public key, meaning it has never had an outgoing transaction. The account has a balance of " + BRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " BNC. Please double check your recipient address before submitting.",
                        "account": response
                    });
                }
            }
    ...
    NSV.div_send_calc_dividends = function () {
        ...
        if (!(/^\d+$/.test(asset))) {
            ...
            BRS.sendOutsideRequest("/burst?requestType=" + "getAsset", {
                ...
                if (outasset !== "") {
                    if (!(/^\d+$/.test(outasset))) {
                        err_message = "Out Asset ID is invalid. Either enter a valid ID or leave blank to pay in BNC.";
                        ...
                        BRS.sendOutsideRequest("/burst?requestType=" + "getAsset", {
                            ...
                            if (response.errorCode) {
                                err_message = "Incorrect Out Asset ID. Either enter a valid ID or leave blank to pay in BNC.";
                            }
                            else {
                ...
                if (NSV_div_send_out_burst) {
                    output = output.concat("Summary of proposed distribution of  ", amount, "BNC to ", String(aa_len),"\n");
                }
                else {
        ...
        if (NSV_div_send_out_burst) {
            ...
            if (BRS.accountInfo.unconfirmedBalanceNQT) {
                ...
                if (balance < (len*100000000)) {
                    err_message = "You don't have enough BNC in this account to pay the fees for this distribution. ".concat(String(len),"BNC needed.");
                }
            }
            else {
                err_message = "This account doesn't seem to have any funds.";
                                ...
                                if (response.timestamp <= final_time) {
                                    if (cur_asset == response.attachment.asset) {
                                        ...
                                        if (response.senderRS == cur_account) {
                                            NSV_div_send_acc_array[cur_index].amount -= parseInt(response.attachment.quantityQNT,10);
                                            if (response.recipientRS != "BNC-NU58-Z4QR-XXKE-94DHH") {	//genesis (86281612813136630310)
    ...
    NSV.shareswap_calc = function() {
        ...
        if (NSV_redeemed_assets.length === 0) {
            ...
            if (num_tobe_swapped > 0) {
                out_message = out_message + "BNC needed for transactions: " + num_tobe_swapped.toString() + ", Replacement Assets needed: " + total_replacement_assets.toString() + "\n";
