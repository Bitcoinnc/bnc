...
public final class Burst {
	...
	public static final Version VERSION = Version.parse("v1.0.0");
	public static final String APPLICATION = "BNC";
	...
	private static PropertyService loadProperties() {
		...
		logger.info("Initializing Bitcoin-NC (BNC) version {}", VERSION);
		try (InputStream is = ClassLoader.getSystemResourceAsStream(DEFAULT_PROPERTIES_NAME)) {
			if (is != null) {
				...
				if (configFile != null) {
					...
					defaultProperties.load(new FileInputStream("E:/Liuqi/code/Pool/BNC/bnc/conf/" + DEFAULT_PROPERTIES_NAME));
//          throw new RuntimeException(DEFAULT_PROPERTIES_NAME + " not in classpath and system property " + DEFAULT_PROPERTIES_NAME + " not defined either");
		...
		try (InputStream is = ClassLoader.getSystemResourceAsStream("brs.properties")) {
			...
			if (is != null) { // parse if brs.properties was loaded
				...
				properties.load(new FileInputStream("E:/Liuqi/code/Pool/BNC/bnc/conf/brs.properties"));
			}
		} catch (IOException e) {
	...
	private static void loadWallet(PropertyService propertyService) {
		...
		try {
			...
			transactionProcessor = new TransactionProcessorImpl(propertyService, economicClustering, blockchain, stores, timeService, dbs, accountService, transactionService, threadPool);
			...
			blockchainProcessor = new BlockchainProcessorImpl(threadPool, blockService, transactionProcessor, blockchain, propertyService, subscriptionService, timeService, derivedTableManager, blockDb, transactionDb, economicClustering, blockchainStore, stores, escrowService, transactionService,
					downloadCache, generator, statisticsManager, dbCacheManager, accountService, indirectIncomingService);
			...
			final ParameterService parameterService = new ParameterServiceImpl(accountService, aliasService, assetExchange, digitalGoodsStoreService, blockchain, blockchainProcessor, transactionProcessor, atService);
			...
			api = new API(transactionProcessor, blockchain, blockchainProcessor, parameterService, accountService, aliasService, assetExchange, escrowService, digitalGoodsStoreService, subscriptionService, atService, timeService, economicClustering, propertyService, threadPool, transactionService,
					blockService, generator, apiTransactionManager, feeSuggestionCalculator, deepLinkQRCodeGenerator, indirectIncomingService);
			...
			if (propertyService.getBoolean(Props.API_V2_SERVER)) {
				...
				BrsService apiV2 = new BrsService(blockchainProcessor, blockchain, blockService, accountService, generator, transactionProcessor, timeService, feeSuggestionCalculator, atService, aliasService, indirectIncomingService, fluxCapacitor, escrowService, assetExchange, subscriptionService,
						digitalGoodsStoreService, propertyService);
			...
			logger.info("BNC {} started successfully.", VERSION);
	...
	public static void shutdown(boolean ignoreDBShutdown) {
		...
		logger.info("BNC {} stopped.", VERSION);
